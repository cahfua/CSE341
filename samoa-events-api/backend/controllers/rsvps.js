...
import db from "../db/connect.js"; // change this path to whatever your project uses
...
export const createRsvp = async (req, res) => {
  try {
    ...
    if (typeof partySize !== "number" || partySize < 1) return res.status(400).json({ message: "partySize must be >= 1" });

    const doc = { eventId, firstName, lastName, phone, partySize, notes: notes || "" };
